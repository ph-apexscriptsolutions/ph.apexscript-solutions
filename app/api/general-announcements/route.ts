      ...
      try {
        ...
        if (!workersError && workers && workers.length > 0) {
          ...
          if (workerEmails.length > 0) {
            ...
            console.log('General announcement emails sent successfully. Result:', result)
          }
        } else if (workersError) {
          console.error('Failed to fetch workers for general announcement emails:', workersError)
        }
      } catch (emailError) {
        console.error('Failed to send general announcement notification emails. Error details:', emailError)
      }
    }
