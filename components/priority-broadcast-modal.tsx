      ...
      <div className={`relative w-full max-w-2xl sm:max-w-3xl overflow-hidden bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-red-200 dark:border-red-900/40 transform transition-all scale-100 ${
        ...
        <div className="bg-gradient-to-r from-red-600 via-rose-600 to-amber-600 px-6 sm:px-8 py-4 sm:py-5 text-white flex items-center justify-between">
          <div className="flex items-center space-x-2.5">
            ...
            <div className="flex items-center space-x-2 font-bold tracking-wide text-sm sm:text-base uppercase">
        ...
        <div className="p-6 sm:p-8 space-y-5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            {timeLeft && (
              <span className={`inline-flex items-center text-xs sm:text-sm font-mono font-bold px-3 py-1 rounded-full ${
          ...
          {/* Title */}
          <div>
            <h3 className="text-2xl sm:text-3xl font-bold text-zinc-900 dark:text-zinc-100 leading-tight">
              {announcement.title}
            </h3>
            <div className="flex items-center gap-2.5 mt-2 flex-wrap">
              <span className="inline-flex items-center gap-1.5 text-xs sm:text-sm font-semibold px-3 py-1 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 border border-zinc-200 dark:border-zinc-700">
          ...
          {isClaimedByOther && (
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800 flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-amber-700">This priority assignment was claimed by {announcement.claimed_by_worker_name || 'another worker'}. Thank you for checking!</p>
              </div>
            </div>
          )}
          ...
          {userResponse && !isClaimedByOther && (
            <div className={`p-4 sm:p-5 rounded-xl text-sm sm:text-base flex items-center justify-between ${
              ...
              <div className="flex items-center space-x-3">
                <CheckCircle className={`h-6 w-6 shrink-0 ${userResponse === 'accepted' ? 'text-emerald-600' : 'text-zinc-500'}`} />
                <div>
                  <p className="font-bold">
                    {userResponse === 'accepted' ? 'You Accepted This Assignment!' : 'You Declined This Assignment'}
                  </p>
                  <p className="text-xs sm:text-sm opacity-80">Admin has been notified via email and live dashboard.</p>
                </div>
              </div>
              <button
                onClick={() => setUserResponse(null)}
                className="text-xs sm:text-sm underline font-medium hover:opacity-80 ml-4 shrink-0"
              >
                Change Answer
              </button>
            </div>
          ...
          {(!userResponse || showNoteInput) && !isClaimedByOther && (
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full text-xs sm:text-sm px-3.5 py-2.5 border border-zinc-300 dark:border-zinc-700 rounded-lg bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-red-500"
              />
            </div>
          )}
          ...
          {!userResponse && !isClaimedByOther && (
            <div className="grid grid-cols-2 gap-4 pt-2">
              <button
                ...
                className="w-full py-3.5 px-5 rounded-xl border border-zinc-300 dark:border-zinc-700 text-zinc-700 dark:text-zinc-300 font-semibold text-sm sm:text-base hover:bg-zinc-100 dark:hover:bg-zinc-800 active:scale-[0.98] transition-all disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={submitting || timeLeft === 'EXPIRED'}
                className="w-full py-3.5 px-5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold text-sm sm:text-base shadow-lg shadow-emerald-600/30 hover:from-emerald-500 hover:to-teal-500 active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center space-x-2"
              >
